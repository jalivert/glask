...
1. Kind inference rejects partially applied higher-kinded constructors.
...
2. Parenthesised operator application rejected by the reader.
   `(+) 1 2` over `examples/positive/operators/infix.glask` returns
   `Left "Semantic Error: Missing Operand ..."`. Bare `(+)` infers fine.

3. Overloaded integer literal keeps its representation under annotation.
...
4. Fixity names are comma-separated.
