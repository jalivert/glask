...
1. N-tuple inference crash (n > 2).
   `(1, True, 'a')` over `examples/positive/data/bool.glask` throws
   `Prelude.undefined` from `src/Compiler/TypeSystem/Type/Infer/Expression.hs:241`.
   Pairs infer fine.

2. `double#+` primitive not implemented.
...
3. Kind inference rejects partially applied higher-kinded constructors.
...
4. Parenthesised operator application rejected by the reader.
   `(+) 1 2` over `examples/positive/operators/infix.glask` returns
   `Left "Semantic Error: Missing Operand ..."`. Bare `(+)` infers fine.

5. Overloaded integer literal keeps its representation under annotation.
...
6. Fixity names are comma-separated.
