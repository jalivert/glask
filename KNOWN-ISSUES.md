...
1. Parenthesised operator application rejected by the reader.
   `(+) 1 2` over `examples/positive/operators/infix.glask` returns
   `Left "Semantic Error: Missing Operand ..."`. Bare `(+)` infers fine.

2. Overloaded integer literal keeps its representation under annotation.
...
3. Fixity names are comma-separated.
